...
void Sudoku::setGrid(const int v[9][9])
{
    for (int x = 0; x < 9; ++x)
    {
        for (int y = 0; y < 9; ++y)
...
void Sudoku::getGrid(int v[9][9]) const
{
    for (int x = 0; x < 9; ++x)
    {
        for (int y = 0; y < 9; ++y)
...
void Sudoku::insertPossibility(int x, int y, int v)
{
    grid[x][y].p_values.insert(v);
}

bool Sudoku::removePossibility(int x, int y, int v)
{
    grid[x][y].p_values.erase(v);
    return grid[x][y].p_values.empty();
}

bool Sudoku::numberPossibilities(int x, int y, unsigned int n) const
{
    return grid[x][y].p_values.size() == n;
}

bool Sudoku::checkInSquare(int x, int y, int v)
{
    int _x = x / 3 * 3;
    int _y = y / 3 * 3;
...
bool Sudoku::checkInLines(int x, int y, int v)
{
    for (int i = 0; i < 9; ++i)
    {
        // check if the value exist
...
void Sudoku::removeInSquare(int x, int y, int v)
{
    for (int i = 0; i < 3; ++i)
    {
        for (int j = 0; j < 3; ++j)
...
void Sudoku::removeInLines(int x, int y, int v)
{
    for (int i = 0; i < 9; ++i)
    {
...
bool Sudoku::emptyPossibilities() const
{
    for (int j = 0; j < 9; ++j)
    {
        for (int i = 0; i < 9; ++i)
...
void Sudoku::fillSharedPossibilities()
{
    for(int y=0; y<9; ++y)
    {
        for(int x=0; x<9; ++x)
...
bool Sudoku::recursiveSolve(unsigned int p_l)
{
    // sudoku solved
    if(isSolved())
    {
...
bool Sudoku::solveValue(unsigned int p_nb, unsigned int p_it)
{
    int solved = 0;
